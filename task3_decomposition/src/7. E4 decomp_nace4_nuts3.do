...
This code performs the size decomposition, demeaned at the NACE 4-digit and NUTS3
levels separately (Table 17 in the Appendix).
______________________________________________________________________________*/

*-----------------
* 1. Decomposition (NACE4 and NUTS3)
*-----------------
...
postfile `memhold' year N b_psi b_n_c b_thetabar b_W_c b_beta ///
				   ...
				   using "results/table_decomp_nace4_nuts3", replace
...
// incidental parameters, drop cells with less than k obs	
	...
	egen nobs2 = count(vat_i), by(nuts3_i)
	drop if nobs < $cutoff_ip
	drop if nobs2 < $cutoff_ip

// demeaning (nace4 + nuts3)      
    foreach x in `v' {									
        reghdfe ln`x', a(FE1_`x' = nace4_i FE2_`x' = nuts3_i) resid
